...
if length(varargin) == 0
    ...
    if ulamsampling == 0
        ...
        for i = 1:horizon
            ...
            pomdp.Allobservation{i} = single(observation);
            pomdp.AllnrActions(i) = single(currentActionNum);
            pomdp.AllActions{i} = Actions;
            pomdp.Allreward{i} = single(pomdp.cost*ones(pomdp.nrStates,pomdp.AllnrActions(i)));
            pomdp.Allindnrobservation{i} = single(indnrobservation);
        end
        pomdp.Actions = [];
        pomdp.nrActions = [];
        pomdp.reward = [];
        ...
        for i = 1:horizon
            pomdp.AllnrActions(i) = single(length(pomdp.AllActions{i}));
            pomdp.Allobservation{i} = single(zeros(nrStates,nrStates,pomdp.AllnrActions(i)));
            pomdp.Allindnrobservation{i} = single(zeros(pomdp.AllnrActions(i),1));
            for IthAction = 1:pomdp.AllnrActions(i)       %%notice that here we haven't consider the case of permutation of q-art question
                ...
                for JthQuestion = 1:q
                    pomdp.Allobservation{i}(currentaction{JthQuestion},1:q,IthAction) = single(repmat(PeMatrix{q-1}(JthQuestion,:),length(currentaction{JthQuestion}),1));
                    %notice pomdp.observation must be nrS x nrO x nrA!!!
                end
                pomdp.Allindnrobservation{i}(IthAction) = single(q);
            end
            pomdp.Allreward{i} = single(pomdp.cost*ones(pomdp.nrStates,pomdp.AllnrActions(i)));
        end
...
elseif length(varargin) == 1
    ...
    if ulamsampling == 0
        ...
        pomdp.Actions =  pomdp.AllActions{1};
        pomdp.AllnrActions = single(pomdp.AllnrActions(i)*ones(horizon,1));
        pomdp.nrActions = pomdp.AllnrActions(1);
        pomdp.reward = pomdp.Allreward{1};
        pomdp.observation = pomdp.Allobservation{1};
        pomdp.Allobservation{1} = [];
        pomdp.indnrobservation  = pomdp.Allindnrobservation{1};
        pomdp.Allindnrobservation{1} = [];
    else
        %% currently we store actions for each horizon wholly. we sampling it by getting sequences of actions in each trial.
        numm = zeros(horizon,1);
        ssize = ceil(wantnum/length(qq))*ones(length(qq),1);
        ...
        for i = 1:horizon
            pomdp.AllnrActions(i) = single(length(pomdp.AllActions{i}));
            pomdp.Allobservation{i} = single(zeros(nrStates,pomdp.nrObservations,pomdp.AllnrActions(i)));
            pomdp.Allindnrobservation{i} = single(zeros(pomdp.AllnrActions(i),1));
            for IthAction = 1:pomdp.AllnrActions(i)       %%notice that here we haven't consider the case of permutation of q-art question
                currentaction = pomdp.AllActions{i}{IthAction};
                q = length(currentaction);
...
elseif length(varargin) == 2  %% THis is sampling with a specific q and e
    ...
    pomdp.AllnrActions(horizon+1) = pomdp.nrStates;
end
